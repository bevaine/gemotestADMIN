...
class KkmUsersController extends Controller
{
    ...
    public function actionExport($id)
    {
        ...
        if ($findModel = $this->findModel($id)) {
            $fio = $findModel->logins->operators->fio;
            if (empty($fio)) $fio = $findModel->logins->Name;
            ...
            <courier dt="2018-04-13 01:00:05">
              ...
              <auth link="auth.xml"> 
                ...
                <adm name="Администратор" login="0000" hash="92AAAB09CA2412A82A2FBF5CDC2DB340B9A3FA79" type="SHA1" mode="A" mask="x00FF"/>
                <seller name="{$fio}" login="{$findModel->login}" mode="K" mask="x0001" pass="{$findModel->password}"/>
              </auth>
